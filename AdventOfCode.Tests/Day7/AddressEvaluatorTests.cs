...
{
    [TestFixture]
    public class AddressEvaluatorTests
    {
        private AddressEvaluator addressEvaluator;

        [SetUp]
        public void Setup()
        {
            addressEvaluator = new AddressEvaluator();
        }

        [TestCase("abba[mnop]qrst", true)]
        [TestCase("axbba[mnop]qrst", false)]
        [TestCase("abbca[mnop]qrst", false)]
        [TestCase("abccba[mnop]qrst", true)]
        [TestCase("abbccba[mnop]qrst", true)]
        [TestCase("babb[amnop]qrst", false)]
        [TestCase("abbba[mnop]qrst", false)]
        [TestCase("abcd[bddb]xyyx", false)]
        [TestCase("abcd[efgh]hijk[bddb]xyyx", false)]
        [TestCase("aaaa[qwer]tyui", false)]
        [TestCase("ioxxoj[asdfgh]zxcvbn", true)]
        [TestCase("ioxxoj[asdfgh]zxcettevbn[adfbsrndabsd]vpoowunbopin", true)]
        ...
        public void AddressSupportsTLS(string address, bool shouldSupportTLS)
        {
            var supportsTLS = addressEvaluator.AllowsTLS(address);
            Assert.That(supportsTLS, Is.EqualTo(shouldSupportTLS));
        }
...
bwzsacxgqkbjycgfw[dbnligvrmqscasutn]rbgybqqsgjvlonkut";
        ...
        {
            var input = GetInput();
            var addresses = input.Split('\r', '\n').Where(i => !string.IsNullOrEmpty(i));
            var addressesSupportingTLS = addresses.Where(a => addressEvaluator.AllowsTLS(a));

            var count = addressesSupportingTLS.Count();
            Assert.That(count, Is.GreaterThan(91));
            Assert.That(count, Is.LessThan(133));
            Assert.Pass($"There are {addressesSupportingTLS.Count()} addresses that support TLS");
        }

        [TestCase("abba[mnop]qrst", false)]
        [TestCase("axbba[mnop]qrst", false)]
        [TestCase("abbca[mnop]qrst", false)]
        [TestCase("abccba[mnop]qrst", false)]
        [TestCase("abbccba[mnop]qrst", false)]
        [TestCase("babb[amnop]qrst", false)]
        [TestCase("abbba[mnop]qrst", false)]
        [TestCase("abcd[bddb]xyyx", false)]
        [TestCase("abcd[efgh]hijk[bddb]xyyx", false)]
        ...
        public void AddressSupportsSSL(string address, bool shouldSupportSSL)
        {
            ...
            Assert.That(supportsSSL, Is.EqualTo(shouldSupportSSL));
        }
        ...
        public void DAY_7_2()
        {
            var input = GetInput();
            var addresses = input.Split('\r', '\n').Where(i => !string.IsNullOrEmpty(i));
            var addressesSupportingSSL = addresses.Where(a => addressEvaluator.AllowsSSL(a));

            var count = addressesSupportingSSL.Count();
            Assert.That(count, Is.GreaterThan(0));
            Assert.That(count, Is.LessThan(2000));
            Assert.Pass($"There are {addressesSupportingSSL.Count()} addresses that support SSL");
        }
    }
}
