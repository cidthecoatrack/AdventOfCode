...
{
    [TestFixture]
    public class RoomDecryptorTests
    {
        private RoomDecryptor roomDecryptor;

        [SetUp]
        public void Setup()
        {
            roomDecryptor = new RoomDecryptor();
        }

        [TestCase("aaaaa-bbb-z-y-x-123[abxyz]", true)]
        [TestCase("aaaaa-bbb-z-y-x-123[baxyz]", false)]
        [TestCase("a-b-c-d-e-f-g-h-987[abcde]", true)]
        [TestCase("not-a-real-room-404[oarel]", true)]
        [TestCase("totally-real-room-200[decoy]", false)]
        public void RoomIsReal(string name, bool shouldBeValid)
        {
            var isValid = roomDecryptor.IsRealRoom(name);
            Assert.That(isValid, Is.EqualTo(shouldBeValid));
        }

        [TestCase("aaaaa-bbb-z-y-x-123[abxyz]", 123)]
        [TestCase("aaaaa-bbb-z-y-x-123[baxyz]", 123)]
        [TestCase("a-b-c-d-e-f-g-h-987[abcde]", 987)]
        [TestCase("not-a-real-room-404[oarel]", 404)]
        [TestCase("totally-real-room-200[decoy]", 200)]
        public void SectorIDs(string name, int expectedSectorID)
        {
            var sectorID = roomDecryptor.GetSectorID(name);
            Assert.That(sectorID, Is.EqualTo(expectedSectorID));
        }

        [Test]
        public void TotalOfSectorIDs()
        {
            var names = new[]
            {
            ...
            var realNames = names.Where(n => roomDecryptor.IsRealRoom(n));
            var sectorIDSum = realNames.Select(n => roomDecryptor.GetSectorID(n)).Sum();
            Assert.That(sectorIDSum, Is.EqualTo(1514));
        }
        ...
        public void DAY_4_1()
        {
            ...
            var realNames = names.Where(n => roomDecryptor.IsRealRoom(n));
            var sectorIDSum = realNames.Select(n => roomDecryptor.GetSectorID(n)).Sum();
            Assert.Pass($"Sector ID sum is {sectorIDSum}");
        }

        [TestCase("qzmt-zixmtkozy-ivhz-343[asdfg]", "very encrypted name")]
        public void DecryptName(string encryptedName, string expectedName)
        {
            var decryptedName = roomDecryptor.DecryptName(encryptedName);
            Assert.That(decryptedName, Is.EqualTo(expectedName));
        }

        [Test]
        public void GetDecryptedRoomNames()
        {
            var names = GetInputs();

            var realNames = names.Where(n => roomDecryptor.IsRealRoom(n));
            var decryptedNames = realNames.Select(n => roomDecryptor.DecryptName(n));

            var possibleMatches = decryptedNames.Where(n => n.Contains("north")
                || n.Contains("pole")
                || n.Contains("santa")
                || n.Contains("claus")
            );

            Assert.That(possibleMatches, Is.Not.Empty);
            Assert.Pass($"Pssible matches are: {string.Join(", ", possibleMatches)}");
        }
        ...
        public void DAY_4_2()
        {
            ...
            {
                var decryptedName = roomDecryptor.DecryptName(name);
                var sectorID = roomDecryptor.GetSectorID(name);

                if (decryptedName.Contains("north"))
                    Assert.Pass($"Sector ID is {sectorID}");
            }

            Assert.Fail("Didn't find the north pole room");
        }
    }
}
